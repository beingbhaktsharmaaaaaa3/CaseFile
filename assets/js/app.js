...
(function () {
  ...
  const contentCache = {}; // slug -> markdown text (populated lazily / via prefetch)
  ...
  function githubEditUrl(node) {
    if (!CONFIG.githubRepoUrl) return null;
    const base = CONFIG.githubRepoUrl.replace(/\/$/, '');
    if (node.hasContent) {
      return `${base}/edit/${CONFIG.branch}/${node.contentPath}`;
    }
    return `${base}/new/${CONFIG.branch}?filename=${encodeURIComponent(node.contentPath)}`;
  }
  ...
  async function fetchContent(node) {
    if (contentCache[node.slug] !== undefined) return contentCache[node.slug];
    try {
      const res = await fetch(node.contentPath, { cache: 'no-store' });
      if (!res.ok) { contentCache[node.slug] = null; return null; }
      const text = await res.text();
      contentCache[node.slug] = text;
      return text;
    } catch (e) {
      contentCache[node.slug] = null;
      return null;
    }
  }
